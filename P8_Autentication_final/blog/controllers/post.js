...
exports.attachment = (req, res, next) => {
    ...
    const { attachment } = post; // const attachment = req.load.post.attachment; (mirar valor en la DB)

    if (!attachment) {
        res.redirect("/images/none.png");
    }
    else if (attachment.image) { // si la imagen esta en el ordenador
        res.type(attachment.mime);
        res.send(Buffer.from(attachment.image.toString(), 'base64'));
    }
    else if (attachment.url) { // si la imagen esta en la red
        res.redirect(attachment.url);
    }
    else { // si no esta en ningun sitio
        res.redirect("/images/none.png");
    }
};

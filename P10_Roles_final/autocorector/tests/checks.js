...
describe("Tests Práctica 10", function() {
    ...
    describe("Tests funcionales", function () {
        ...
        before(async function() {
            ...
            server = spawn('node', [bin_path], {env: {DEBUG: DEBUG, PORT: TEST_PORT, DATABASE_URL: db_relative_url, PATH: process.env.PATH}});
        ...
        scored(`El botón de editar un post aparece si el usuario logueado es el autor del post.`, 0.2, async function(){
            await allUsers.apply(this, [async function(user) {
                ...
                debug("POST CREADO. URL devuelta: " + post_path);
                ...
                try { 
                    ...
                    debug("No se pudo borrar el post", e)
                }
            }]);
        });
